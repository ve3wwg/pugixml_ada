...
extern "C" {
	...
	void pugi_load_xml_file(pugi::xml_document *obj,const char *pathname,int *status,unsigned *offset,int *encoding,int *ok,const char **desc,unsigned& options,int enc_opt);
	void pugi_load_in_place(pugi::xml_document *obj,void *content,int *status,unsigned *offset,int *encout,int *ok,const char **desc,unsigned& options,int size,int encoding);
	int pugi_save_file(pugi::xml_document *obj,const char *pathname,const char *indent,int encoding,unsigned& format);
...
void
pugi_load_xml_file(pugi::xml_document *obj,const char *pathname,int *status,unsigned *offset,int *encoding,int *ok,const char **desc,unsigned& options,int enc_opt) {
	
	pugi::xml_parse_result r = obj->load_file(pathname,options,pugi::xml_encoding(enc_opt));
	*status = int(r.status);
	*offset = r.offset;
...
void
pugi_load_in_place(pugi::xml_document *obj,void *content,int *status,unsigned *offset,int *encout,int *ok,const char **desc,unsigned& options,int size,int encoding) {
        
        pugi::xml_parse_result r = obj->load_buffer_inplace(content,size_t(size),options,pugi::xml_encoding(encoding));
        *status = int(r.status);
        *offset = r.offset;
...
int
pugi_save_file(pugi::xml_document *obj,const char *pathname,const char *indent,int encoding,unsigned& format) {
        return obj->save_file(pathname,indent,format,pugi::xml_encoding(encoding)) ? 1 : 0;
}
